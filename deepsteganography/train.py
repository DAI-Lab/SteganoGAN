...
class ModelTrainer(object):
    ...
    def __init__(self, dataset, data_depth, hidden_dim, batch_size=4, lr=1e-4):
    ...
    def step(self):
        self.epoch += 1
        d_train_loss, c_train_loss = self._train(self.train_set)
        d_test_acc, d_test_loss, c_test_loss = self._test(self.test_set)

        torch.save(self.model, self._log_dir + "epoch-%s.loss-%.03f.pt" % (self.epoch, d_test_loss))
...
if __name__ == "__main__":
    trainer = ModelTrainer(args.dataset, args.data_depth, args.hidden_dim)
