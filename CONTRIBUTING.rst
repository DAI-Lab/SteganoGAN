...
Deep Steganography could always use more documentation, whether as part of the
official Deep Steganography docs, in docstrings, or even on the web in blog posts,
articles, and such.
...
Ready to contribute? Here's how to set up `Deep Steganography` for local development.

1. Fork the `Deep Steganography` repo on GitHub.
...
2. The tests that cover a module called ``deepsteganography/path/to/a_module.py``
   should be implemented in a separated module called
   ``tests/deepsteganography/path/to/test_a_module.py``.
...
2. Update the version in ``setup.cfg``, ``deepsteganography/__init__.py`` and
   ``HISTORY.md`` files.
3. Create a new git tag pointing at the corresponding commit in ``stable`` branch.
4. Merge the new commit from ``stable`` into ``master``.
5. Update the version in ``setup.cfg`` and ``deepsteganography/__init__.py``
...
.. _GitHub issues page: https://github.com/DAI-Lab/deepsteganography/issues
.. _Travis Build Status page: https://travis-ci.org/DAI-Lab/deepsteganography/pull_requests
