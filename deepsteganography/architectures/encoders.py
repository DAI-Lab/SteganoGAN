...
class DenseEncoder(nn.Module):
    ...
    def __init__(self, data_depth, hidden_size):
        ...
        self.conv3 = nn.Sequential(
            nn.Conv2d(in_channels=hidden_size*2 + data_depth, out_channels=hidden_size, kernel_size=3, padding=1),
            nn.LeakyReLU(inplace=True),
            nn.BatchNorm2d(hidden_size),
        )
        self.conv4 = nn.Sequential(
            nn.Conv2d(in_channels=hidden_size*3 + data_depth, out_channels=3, kernel_size=3, padding=1),
        )
